...
export class CrearSolicitudComponent {
  max_date = moment(new Date().getFullYear().toString() + '-12-31').format(
    'YYYY-MM-DD'
  );
  ...
  ngOnInit(): void {
    ...
    if (fecha_actual.month() === 12 && fecha_actual.date() >= 16) {
      ...
      this.max_date = moment(nuevo_año.toString() + '-12-31').format(
        'YYYY-MM-DD'
      );
    }
      ...
      this.trabajadorService.getEmpleadoByUserId(id_usuario).subscribe({
        ...
        error: (err) => {
          ...
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'Huoo un error al completarse la solicitud',
            confirmButtonColor:'#198754'
          });
          console.log(err);
          ...
          this.festivosService.getDiasFeriados().subscribe({
            ...
            error: (err) => {
              ...
              Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Hubo un problema al obtener los días feriados',
              });
              console.log(err);
  ...
  crearFormulario() {
    this.solicitud_formulario = this.fb.group(
      ...
      {
        validators: [
          this.maxDateValidator('fecha_inicio', 'fecha_fin'),
          this.rangeDateValidator('fecha_inicio', 'fecha_fin'),
        ],
      }
    );
  }
  ...
  crearSolicitud() {
    if (!this.solicitud_formulario.invalid) {
      ...
      return Object.values(this.solicitud_formulario.controls).forEach(
        (control) => {
  ...
  get justificacionNoValida() {
    this.mensaje = '';
    if (this.solicitud_formulario.get('justificacion')?.invalid) {
      return true;
    } else {
      return false;
    }
